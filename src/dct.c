...
void DCT16x(int *data, int *spec) {
  ...
  spec[0] =
      (int)((g_aiT16[0][0] * EEE[0] + g_aiT16[0][1] * EEE[1] + add) >> SHIFT_X);
  spec[8] =
      (int)((g_aiT16[8][0] * EEE[0] + g_aiT16[8][1] * EEE[1] + add) >> SHIFT_X);
  spec[4] =
      (int)((g_aiT16[4][0] * EEO[0] + g_aiT16[4][1] * EEO[1] + add) >> SHIFT_X);
  spec[12] = (int)((g_aiT16[12][0] * EEO[0] + g_aiT16[12][1] * EEO[1] + add) >>
                   SHIFT_X);

  for (k = 2; k < 16; k += 4) {
    spec[k] = (int)((g_aiT16[k][0] * EO[0] + g_aiT16[k][1] * EO[1] +
                     g_aiT16[k][2] * EO[2] + g_aiT16[k][3] * EO[3] + add) >>
                    SHIFT_X);
  }

  for (k = 1; k < 16; k += 2) {
    spec[k] = (int)((g_aiT16[k][0] * O[0] + g_aiT16[k][1] * O[1] +
                     g_aiT16[k][2] * O[2] + g_aiT16[k][3] * O[3] +
                     g_aiT16[k][4] * O[4] + g_aiT16[k][5] * O[5] +
                     g_aiT16[k][6] * O[6] + g_aiT16[k][7] * O[7] + add) >>
                    SHIFT_X);
  }
}
...
void DCT16y(int *data, int *spec) {
  ...
  spec[0 * 16] =
      (int)((g_aiT16[0][0] * EEE[0] + g_aiT16[0][1] * EEE[1] + add) >> SHIFT_Y);
  spec[8 * 16] =
      (int)((g_aiT16[8][0] * EEE[0] + g_aiT16[8][1] * EEE[1] + add) >> SHIFT_Y);
  spec[4 * 16] =
      (int)((g_aiT16[4][0] * EEO[0] + g_aiT16[4][1] * EEO[1] + add) >> SHIFT_Y);
  spec[12 * 16] =
      (int)((g_aiT16[12][0] * EEO[0] + g_aiT16[12][1] * EEO[1] + add) >>
            SHIFT_Y);

  for (k = 2; k < 16; k += 4) {
    spec[k * 16] =
        (int)((g_aiT16[k][0] * EO[0] + g_aiT16[k][1] * EO[1] +
               g_aiT16[k][2] * EO[2] + g_aiT16[k][3] * EO[3] + add) >>
              SHIFT_Y);
  }

  for (k = 1; k < 16; k += 2) {
    spec[k * 16] = (int)((g_aiT16[k][0] * O[0] + g_aiT16[k][1] * O[1] +
                          g_aiT16[k][2] * O[2] + g_aiT16[k][3] * O[3] +
                          g_aiT16[k][4] * O[4] + g_aiT16[k][5] * O[5] +
                          g_aiT16[k][6] * O[6] + g_aiT16[k][7] * O[7] + add) >>
                         SHIFT_Y);
...
void IDCT16x(int *tmp, int *block) {
   ...
   * multiplications */
  for (k = 0; k < 8; k++) {
    O[k] = g_aiT16[1][k] * tmp[1] + g_aiT16[3][k] * tmp[3] +
           g_aiT16[5][k] * tmp[5] + g_aiT16[7][k] * tmp[7] +
           g_aiT16[9][k] * tmp[9] + g_aiT16[11][k] * tmp[11] +
           g_aiT16[13][k] * tmp[13] + g_aiT16[15][k] * tmp[15];
  }

  for (k = 0; k < 4; k++) {
    EO[k] = g_aiT16[2][k] * tmp[2] + g_aiT16[6][k] * tmp[6] +
            g_aiT16[10][k] * tmp[10] + g_aiT16[14][k] * tmp[14];
  }
